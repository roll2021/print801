...
GPIO_TypeDef *LED_GPIO_PORT[LED_NUM] = {LED1_GPIO, LED2_GPIO, LED3_GPIO, LED4_GPIO};
...
GPIO_TypeDef *BUTTON_GPIO_PORT[BUTTON_NUM] = {BUTTON_WAKEUP_GPIO, BUTTON_USER_KEY_GPIO};
...
{  
  ...
  while ( USART_GetFlagStatus(AT32_PRINT_UART, USART_FLAG_TC) == RESET );    
  return ch;
}
...
void UART_Print_Init(uint32_t bound)
{
  ...
  GPIO_StructInit(&GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin = AT32_PRINT_UARTTX_PIN; 
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	
  GPIO_Init(AT32_PRINT_UARTTX_GPIO, &GPIO_InitStructure);

  /* Configure the UART1 RX pin */
  GPIO_InitStructure.GPIO_Pin = AT32_PRINT_UARTRX_PIN;//PA10
  ...
  USART_ITConfig(AT32_PRINT_UART, USART_IT_RXNE, ENABLE);
