...
void SPI_Init(SPI_TypeDef* SPIx, SPI_InitTypeDef* SPI_InitStruct)
{
  ...
  if (SPI_InitStruct->SPI_BaudRatePrescaler & SPI_BaudRatePrescaler_OVER_256)
  {
    /* MCLKP is over 256 */
    SPIx->CR2 |= SPI_CR2_MCLKP_3;
  }
  else
  {
    SPIx->CR2 &= ~SPI_CR2_MCLKP_3;
  ...
  tmpreg |= (uint16_t)((uint32_t)SPI_InitStruct->SPI_Direction | SPI_InitStruct->SPI_Mode |
                       ...
                       (SPI_InitStruct->SPI_BaudRatePrescaler & 0x7FFF) | SPI_InitStruct->SPI_FirstBit);
  /* Write to SPIx CR1 */
  SPIx->CR1 = tmpreg;
