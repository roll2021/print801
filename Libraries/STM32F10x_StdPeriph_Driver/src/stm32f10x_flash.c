...
#define FLASH_BANK3_START_ADDRESS ((uint32_t)0x8400000)

/* Delay definition */
#define EraseTimeout          ((uint32_t)0x10000000)
#define ProgramTimeout        ((uint32_t)0x0000F000)
#define Bank3_EraseTimeout    ((uint32_t)0xFFFFFFFF)
#define Bank3_ProgramTimeout  ((uint32_t)0x00080000)
...
#endif /* STM32F10X_XL */
/**
  * @brief  Unlocks the FLASH Bank3 Program Erase Controller.
  * @note   This function can be used only for STM32F10X_XL density devices.
  * @param  None
  * @retval None
  */
...
{
  /* Authorize the FPEC of Bank2 Access */
  FLASH->KEYR3 = FLASH_KEY1;
  FLASH->KEYR3 = FLASH_KEY2;
...
#endif /* STM32F10X_XL */
/**
  * @brief  Locks the FLASH Bank3 Program Erase Controller.
  * @note   This function can be used only for SPIM.
  * @param  None
  * @retval None
  */
...
FLASH_Status FLASH_ErasePage(uint32_t Page_Address)
{
  ...
  if(Page_Address >= FLASH_BANK3_START_ADDRESS)
  {
    /* Wait for last process to be completed */
    status = FLASH_WaitForLastBank3Operation(Bank3_EraseTimeout);

    if(status == FLASH_COMPLETE)
    {
      /* if the previous process is completed, continue to erase the page */
      FLASH->CR3 |= CR_PER_Set;
      FLASH->AR3 = Page_Address;
      FLASH->CR3 |= CR_STRT_Set;

      /* Wait for last process to be completed */
      status = FLASH_WaitForLastBank3Operation(Bank3_EraseTimeout);

      /* Disable the PGERS Bit */
      FLASH->CR3 &= CR_PER_Reset;
    }
...
/**
  ...
  * @note   This function can be used for SPIM.
  * @param  None
  * @retval FLASH Status: The returned value can be: FLASH_ERROR_PG,
  *         FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
FLASH_Status FLASH_EraseAllBank3Pages(void)
{
  FLASH_Status status = FLASH_COMPLETE;
  /* Wait for last process to be completed */
  status = FLASH_WaitForLastBank3Operation(Bank3_EraseTimeout);
...
FLASH_Status FLASH_ProgramWord(uint32_t Address, uint32_t Data)
{
  ...
  if (Address >= FLASH_BANK3_START_ADDRESS)
  {
    /* Wait for last process to be completed */
    status = FLASH_WaitForLastBank3Operation(Bank3_ProgramTimeout);
...
void FLASH_ITConfig(uint32_t FLASH_IT, FunctionalState NewState)
{
  ...
  if((FLASH_IT & 0x40000000) != 0x0)//SPIM
  {
    if(NewState != DISABLE)
    {
      /* Enable the interrupt sources */
      FLASH->CR3 |= (FLASH_IT & 0xBFFFFFFF);
    }
    else
    {
      /* Disable the interrupt sources */
      FLASH->CR3 &= ~(uint32_t)(FLASH_IT & 0xBFFFFFFF);
    }
  }
  else if((FLASH_IT & 0x80000000) != 0x0)
  {
    if(NewState != DISABLE)
    {
      /* Enable the interrupt sources */
...
FlagStatus FLASH_GetFlagStatus(uint32_t FLASH_FLAG)
{
  ...
  else
  {
    ...
    else if((FLASH_FLAG & 0x40000000) != 0x0)//SPIM
    {
      if((FLASH->SR3 & FLASH_FLAG) != (uint32_t)RESET)
      {
        bitstatus = SET;
      }
      else
  ...
  {
    if((FLASH_FLAG & 0x40000000) != 0x0)//SPIM
    {
      if((FLASH->SR3 & FLASH_FLAG) != (uint32_t)RESET)
      {
        bitstatus = SET;
...
void FLASH_ClearFlag(uint32_t FLASH_FLAG)
{
  ...
  {
    /* Clear the flags */
    FLASH->SR3 = FLASH_FLAG;
  }
  else if((FLASH_FLAG & 0x80000000) != 0x0)
  {
    /* Clear the flags */
...
/**
  * @brief  Returns the FLASH Bank1 Status.
  * @note   This function can be used for SPIM, it is equivalent
  *         to FLASH_GetStatus function.
  * @param  None
  * @retval FLASH Status: The returned value can be: FLASH_BUSY, FLASH_ERROR_PG,
  *         FLASH_ERROR_WRP or FLASH_COMPLETE
...
FLASH_Status FLASH_GetBank3Status(void)
{
  ...
  if((FLASH->SR3 & FLASH_FLAG_BANK3_BSY) == FLASH_FLAG_BSY)
  {
    flashstatus = FLASH_BUSY;
  }
  else
  {
    if((FLASH->SR3 & FLASH_FLAG_BANK3_PGERR) != 0)
    {
      flashstatus = FLASH_ERROR_PG;
    }
    else
    {
      if((FLASH->SR3 & FLASH_FLAG_BANK3_WRPRTERR) != 0 )
      {
        flashstatus = FLASH_ERROR_WRP;
      }
      else
...
#endif /* STM32F10X_XL */
/**
  * @brief  Waits for a Flash operation on Bank3 to complete or a TIMEOUT to occur.
  * @note   This function can be used for SPIM,
  *         it is equivalent to FLASH_WaitForLastOperation.
  * @param  Timeout: FLASH programming Timeout
  * @retval FLASH Status: The returned value can be: FLASH_ERROR_PG,
  *         FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
FLASH_Status FLASH_WaitForLastBank3Operation(uint32_t Timeout)
{
  FLASH_Status status = FLASH_COMPLETE;
  ...
  /* Wait for a Flash operation to complete or a TIMEOUT to occur */
  while((status == FLASH_FLAG_BANK3_BSY) && (Timeout != 0x00))
  {
    status = FLASH_GetBank3Status();
    Timeout--;
  }
