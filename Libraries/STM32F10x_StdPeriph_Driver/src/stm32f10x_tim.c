...
void TIM_TimeBaseInit(TIM_TypeDef* TIMx, TIM_TimeBaseInitTypeDef* TIM_TimeBaseInitStruct)
{
  ...
  (TIMx == TIM15) ||
#endif
  (TIMx == TIM2) || (TIMx == TIM3) || (TIMx == TIM4) || (TIMx == TIM5))
  {
...
/**
  ...
  * @retval None
  */
void TIM_SelectPlusMode(TIM_TypeDef* TIMx, uint16_t TIMx_PLUS_MODE_STATE)
{
	/* Check the parameters */
  assert_param(IS_TIM_PLUSMODE_PERIPH(TIMx));
	assert_param(IS_TIM_PLUS_MODE_STATE(TIMx_PLUS_MODE_STATE));
	/*Config TIMx plus mode*/
	if((TIMx == TIM2) || (TIMx == TIM5))
	{
		/*Enable*/
		if(TIMx_PLUS_MODE_STATE)
		{
			TIMx->CR1 &= (uint16_t)(~((uint16_t)TIM_Plus_Mode_Enable));
			TIMx->CR1 |= TIM_Plus_Mode_Enable;
		}
		/*Disable*/
		else
		{
			TIMx->CR1 &= (uint16_t)(~((uint16_t)TIM_Plus_Mode_Enable));
			TIMx->CR1 |= TIM_Plus_Mode_Disable;
		}
	}
}
