...
//    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLMULL15 | RCC_CFGR_PLLRANGE_GT72MHZ);
		...
		/* PLL configuration: PLLCLK = HSE /5 * 24 = 120 MHz  先5分频再倍频到120Mhz */ 
		*((unsigned int *)0x40021054) |= (0x3000);// 
		RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE | RCC_CFGR_PLLMULL));
		RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | RCC_CFGR_PLLXTPRE_HSE_Div2| RCC_CFGR_PLLMULL24 | RCC_CFGR_PLLRANGE_GT72MHZ);
